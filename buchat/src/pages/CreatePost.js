...
const CreatePost = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // Logic would go here to handle both File objects and URL objects (GIFs)
      // For demo purposes, we skip the actual upload logic for URL-based media
            ...
            <div className="top-controls-grid">
              <GlassSelect
                ...
                optgroups={[
                  {
                    label: 'Your Tribes',
                    options: groups.map(g => ({ value: g.name, label: `c/${g.name}` }))
                  }
                ]}
              />
